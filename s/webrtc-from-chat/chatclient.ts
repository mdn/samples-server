...
import { IServerMessage } from './chatserver.definitions';
...
let targetUsername: string | null;               // To store username of other peer
let myPeerConnection: RTCPeerConnection;  // RTCPeerConnection
let transceiver: RTCRtpTransceiver;       // RTCRtpTransceiver
...
const log_error = (text: string|Error) => {
  const time = new Date();
...
const sendToServer = (msg: IServerMessage) => {
...
// session.
const setUsername = () => {
  myUsername = (document.getElementById("name") as HTMLInputElement).value;
...
const connect = () => {
  ...
  connection.onerror = evt => console.dir(evt);
...
const handleKey = (evt: KeyboardEvent) => {
  if (evt.keyCode === 13 || evt.keyCode === 14 && !(document.getElementById("send") as HTMLButtonElement).disabled) {
    handleSendButton();
  }
};
...
function handleTrackEvent(event: RTCTrackEvent) {
  log("*** Track event");
  (document.getElementById("received_video") as HTMLVideoElement).srcObject = event.streams[0];
  (document.getElementById("hangup-button") as HTMLButtonElement).disabled = false;
}
...
function handleUserlistMsg(msg: {users: string[]}) {
  const listElem = document.querySelector(".userlistbox") as HTMLUListElement;
...
function closeVideoCall() {
  ...
  (document.getElementById("hangup-button") as HTMLButtonElement).disabled = true;
  // targetUsername = null;
}
...
function handleHangUpMsg(msg: IServerMessage) {
...
async function invite(evt: Event ) {
  log("Starting to prepare an invitation");
  if (myPeerConnection) {
    alert("You can't start a call because you already have one open!");
  } else {
    const clickedUsername = (evt as Event & {target: Node}).target.textContent;
...
async function handleVideoOfferMsg(msg: IServerMessage) {
...
async function handleVideoAnswerMsg(msg: IServerMessage) {
...
async function handleNewICECandidateMsg(msg: IServerMessage) {
